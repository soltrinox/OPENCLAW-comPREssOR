...
`package.json` `openclaw.extensions` and `openclaw.runtimeExtensions` point at `./dist/index.js` for published ClawPack artifacts. Build with `npm run build` (or `npm run pack`) before install from the packed tree. npm / ClawHub package name is `@soltrinox/openclaw-compressor`.
...
Gateway host `2026.7.1-2` does not expose a plugin doctor-check registrar. Do not expect compressor checks under `openclaw doctor`. Run the CLI subcommand instead:
...
Exit code `1` if any finding has severity `fail`; `0` when all pass/warn.
...
# Doctor checks (CLI; not registered on openclaw doctor for host 2026.7.1-2)
openclaw compressor doctor
```
