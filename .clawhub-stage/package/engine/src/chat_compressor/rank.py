...
MIN_RANK_SCORE = 0.03
# recall-0.5 default; cursor-parity sets CHAT_COMPRESSOR_RANK_FALLBACK_TOP_K=3
RANK_FALLBACK_TOP_K = 8


def rank_fallback_top_k(default: int = RANK_FALLBACK_TOP_K) -> int:
    raw = os.environ.get("CHAT_COMPRESSOR_RANK_FALLBACK_TOP_K", "").strip()
    if not raw:
        return int(default)
    try:
...
def rank_relevant_chunks(
    ...
    *,
    ...
    if filtered:
        return filtered
    k_fb = rank_fallback_top_k() if fallback_top_k is None else int(fallback_top_k)
    return ranked[: max(0, k_fb)]
