...
from chat_compressor.graph import CtxGraph, hot_set_max_chars
from chat_compressor.producer import chunks_per_turn, make_producer
...
def matrix_span_readout_enabled() -> bool:
    raw = os.environ.get("CHAT_COMPRESSOR_MATRIX_SPAN_READOUT", "1").strip().lower()
    return raw not in {"0", "false", "no", "off"}


def matrix_span_k(default: int = 8) -> int:
    raw = os.environ.get("CHAT_COMPRESSOR_MATRIX_SPAN_K", "").strip()
    if not raw:
        return int(default)
    try:
        return max(1, int(raw))
...
class PersistentAgentHandle:
    ...
    def step(
        ...
        *,
        ...
        spans = [
            {"text": c, "row": i}
            for i, c in enumerate(chunk_text(new_input, max_chunks=chunks_per_turn()))
            if (c or "").strip()
        ]
    ...
    def sample_for(self, target: str, query: str | None = None) -> SampledPayload:
        """cursor-sdk => packed HOT_SET/typed/ranked/spans text. local:<id> may return C_B floats."""
        node = self.latest()
        q = (query or "").strip() or self._last_user_query()
        hot = self.graph.hot_set(query=q or None, max_chars=hot_set_max_chars())
        ...
        if cross_turn_dedup_enabled() and history:
            ...
            node_superseded = self.graph.supersede_count() > int(
                last.get("supersede_count") or 0
            )
        ...
        span_list: list[str] = []
        if (q or "").strip():
            cands = collect_candidates(self.graph, query=q)
            ranked_list = [r.text for r in rank_relevant_chunks(q, cands)]
        if matrix_span_readout_enabled() and (q or "").strip():
            span_list = self.expand_spans(q, k=matrix_span_k())

        ranked_norm = {x.lower() for x in ranked_list}
        span_list = [s for s in span_list if s.lower() not in ranked_norm]
        ...
        pack_kwargs = {
            ...
            "query": q or None,
            "typed_lines": typed,
            "ranked_chunks": ranked_list + span_list,
            "graph": self.graph,
            "budget": budget,
    ...
    def expand_spans(self, query: str, k: int = 8) -> list[str]:
